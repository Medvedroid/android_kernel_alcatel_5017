...
static uint32 Data_Wait_Queue_flag = 0;
...
static struct file_operations OffloadService_fops =
{
    .owner          = THIS_MODULE,
...
struct dev_pm_ops OffloadService_pm_ops =
{
    .suspend = NULL,
    .resume = NULL,
    .freeze = NULL,
...
{
    int ret = 0;
    printk("OffloadService_mod_init \n");

    // Register platform DRIVER
    ret = platform_driver_register(&OffloadService_driver);
    if (ret)
    {
        printk("OffloadService Fail:%d - Register DRIVER \n", ret);
        return ret;
    }

    // register MISC device
    if ((ret = misc_register(&OffloadService_misc_device)))
    {
        printk("OffloadService misc_register Fail:%d \n", ret);
        return ret;
    }
