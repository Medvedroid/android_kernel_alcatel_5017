...
static struct device *devinfo_msensor;
static int msensor_detect = 0;
...
static ssize_t msensor_devinfo_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
       unsigned char *strid="AKM09911:AKM:SUP1:";
        if(msensor_detect)
               return sprintf(buf, "%s%d\n", strid, msensor_detect);
        else
               return sprintf(buf, "NA:NA:NA:%d\n", msensor_detect);
}
static DEVICE_ATTR(msensor, 0444, msensor_devinfo_show, NULL);
