...
void pmic_voltage_read(unsigned int nAdjust)
{
	...
	pr_info("[PMIC]pmic_voltage_read : \r\n");
	...
	pr_info("[Vcore] MT6328_VCORE1_CON11=0x%x,\r\n[Vmem] MT6328_SLDO_ANA_CON0/1=0x%x 0x%x\r\n",
...
void pmic_Vcore_adjust(int nAdjust)
{
	...
	case 5:		/* LV 938MHz */
		...
		break;
	default:
		pr_info("Enter incorrect config number !!!\n");
		break;
	}
...
void dump_DMA_Reserved_AREA(void)
{
	...
	/*unsigned int *debug_p_src;*/
	   ...
	   pr_info("=====DMA DUMP Reserved SRC AREA======\n");
	   /*  dump memory info to make sure 0x6a6a6a6a already write into memory */
	   for (i = 0; i < 8; i++) {
		pr_info("[SRC] ");
		for (j = 0; j < 8; j++)
			pr_info("0x%p=0x%x ", ((debug_p_src+i*8)+j), *((debug_p_src+i*8)+j));
		/* pr_info("&0x%p=0x%x ", ((debug_p_src+i*8)+j), readl(IOMEM((debug_p_src+i*8)+j))); */
		pr_info("\n");
	   }

	   pr_info("=====DMA DUMP Reserved SRC AREA END======\n\n");

	pr_info("=====DMA DUMP Reserved DST AREA======\n");
	for (i = 0; i < 8; i++) {
		pr_info("[DST] ");
		for (j = 0; j < 8; j += 4) {
			pr_info("val=0x%02X",
			       dfs_dummy_buffer[(i * 8) + (j + 3)]);
			pr_info("%02X", dfs_dummy_buffer[(i * 8) + (j + 2)]);
			pr_info("%02X", dfs_dummy_buffer[(i * 8) + (j + 1)]);
			pr_info("%02X ", dfs_dummy_buffer[(i * 8) + j]);
		}
		/*pr_info("&0x%p=0x%x ", ((debug_p_dst+i*8)+j), readl(IOMEM((debug_p_dst+i*8)+j)));*/
		pr_info("\n");
	}
	pr_info("=====DMA DUMP Reserved DST AREA END======\n\n");

	enable_clock(MT_CG_INFRA_GCE, "CQDMA");
	pr_info("=====DMA DUMP REGS======\n");
	for (k = 0; k < 25; k++) {
		pr_info("[CQDMA] addr:0x%p, value:%x\n",
		       (CQDMA_BASE_ADDR + k * 4),
...
int dram_fh_steps_freq(unsigned int step)
{
	...
	unsigned int temp = (get_devinfo_with_index(5) & (1 << 21)) ? 1 : 0;
...
#else
	pr_info("DRAM FH steps not define\n");
#endif
...
int dram_can_support_fh(void)
{
	...
	/*value = (0x1 <<15);
	pr_info("dummy regs 0x%x\n", value);
	pr_info("check 0x%x\n",(0x1 <<15));*/

	if (value & (0x1 << 15)) {
		/*pr_info("DFS could be enabled\n");*/
		return 1;
	} else {
		/*pr_info("DFS could NOT be enabled\n");*/
		return 0;
	}
}
...
int dram_do_dfs_by_fh(unsigned int freq)
{
	...
	/*pr_info("dram_do_dfs_by_fh to 0x%x ddr_type = %d\n", target_dds, ddr_type);*/
	enable_clock(MT_CG_INFRA_GCE, "CQDMA");
				       ...
				       const char *buf, size_t count)
{
	...
	pr_info("[DRAM DFS] freqency hopping to %dKHz\n", freq);
...
static int dram_dt_init(void)
{
	...
	if (node) {
		...
		pr_info("get APMIXED_BASE_ADDR @ %p\n", APMIXED_BASE_ADDR);
	} else {
	...
	if (node) {
		...
		pr_info("[DRAMC]get CQDMA_BASE_ADDR @ %p\n", CQDMA_BASE_ADDR);
	} else {
		pr_info("[DRAMC]can't find compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_info("[DRAMC]get DRAMCAO_BASE_ADDR @ %p\n", DRAMCAO_BASE_ADDR);
	} else {
		pr_info("[DRAMC]can't find DRAMC0 compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_info("[DRAMC]get DDRPHY_BASE_ADDR @ %p\n", DDRPHY_BASE_ADDR);
	} else {
		pr_info("[DRAMC]can't find DDRPHY compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_info("[DRAMC]get DRAMCNAO_BASE_ADDR @ %p\n", DRAMCNAO_BASE_ADDR);
	} else {
		pr_info("[DRAMC]can't find DRAMCNAO compatible node\n");
		return -1;
	}
...
int __init dram_test_init(void)
{
	...
	ret = dram_dt_init();
	if (ret) {
		pr_info("[DRAMC] Device Tree Init Fail\n");
		return ret;
	}
	...
	if (dram_can_support_fh())
		pr_info(KERN_INFO "[DRAMC Driver] dram can support Frequency Hopping\n");
	else
		pr_info(KERN_INFO "[DRAMC Driver] dram can not support Frequency Hopping\n");
