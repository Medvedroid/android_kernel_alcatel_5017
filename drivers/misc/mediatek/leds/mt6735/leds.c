...
/*[439014] Led is always light in low power. modify liming.zhu at 20150805 bigen*/
extern struct workqueue_struct *led_workqueue;
/*[439014] Led is always light in low power. modify liming.zhu at 20150805 end*/
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	LEDS_DEBUG("[LED]LED blink on time = %d offtime = %d\n",led->blink_on_time,led->blink_off_time);
    /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] bigen*/
    time_index = led->blink_on_time + led->blink_off_time - 1;
    LEDS_DEBUG("[LED]LED index is %d  freqsel=%d\n", time_index, time_index);
    /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] end*/
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			/*[265321] change charge lights current from 16ma to 4ma liming.zhu at 20150527 begin*/
			pmic_set_register_value(PMIC_ISINK_CH0_STEP,ISINK_0);//4mA
			/*[265321] change charge lights current from 16ma to 4ma liming.zhu at 20150527 end*/
			pmic_set_register_value(PMIC_ISINK_DIM0_DUTY,duty);
            /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] bigen*/
            pmic_set_register_value(PMIC_ISINK_DIM0_FSEL,time_index);
            /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] end*/			
			pmic_set_register_value(PMIC_ISINK_CH0_EN,NLED_ON);
			break;
		case MT65XX_LED_PMIC_NLED_ISINK1:
            ...
            /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] bigen*/
            pmic_set_register_value(PMIC_ISINK_DIM1_FSEL,time_index);
            /* [101253] [pixi3-4.5 4g]charging LED indicator blink when there are new events by [liming.zhu.hz] at [2015.0119] end*/		
			pmic_set_register_value(PMIC_ISINK_CH1_EN,NLED_ON);
			break;	
		default:
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if(led_data->level != level)
	{
		...
		if(strcmp(led_data->cust.name,"lcd-backlight") != 0)
		{
			...
			/*[439014] Led is always light in low power. modify liming.zhu at 20150805 bigen*/
			if (NULL != led_workqueue) 
				queue_work(led_workqueue, &led_data->work); 
			else 
