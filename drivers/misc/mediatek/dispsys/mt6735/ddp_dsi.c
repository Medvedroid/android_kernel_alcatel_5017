...
static DEFINE_SPINLOCK(s_lock);//modify by llf for defect 438994
void DSI_sw_clk_trail(int module_idx)
{
	//DEFINE_SPINLOCK(s_lock);//modify by llf for defect 438994
	unsigned long flags;
...
//	unsigned int fmod = 30;//Fmod = 30KHz by default
	unsigned int delta1 = 5;//Delta1 is SSC range, default is 0%~-5%//modify by llf for Defect 570091 return
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	{	
		...
		for(i = 0; i < 3; i++)
		{
			...
			// write DSI CMDQ add by llf for esd check
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], 0x00063902); 
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[1], 0x0698ffff);

			if(dsi_params->lcm_esd_check_table[i].cmd == 0x0A)
			{ 
				//page 0 
				DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[2], 0x00000004); 
				//printk("[DSI_LLF]enter page0 change\n");

			}else//index ==0;
			{
				//page 6
				DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[2], 0x00000604); 
				//printk("[DSI_LLF]enter page6 change\n");
			}

			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[3], 0x00013700);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[4], AS_UINT32(&t0));
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_CMDQ_SIZE, 5);
/*
...
*/
			//add by llf for esd check
			printk("[DSI_LLF_ESDCHECK_DATA]enter cmp read_data0 REG=0x%x value=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,read_data0.byte1);
