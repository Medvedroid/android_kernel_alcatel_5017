...
static int alsps_real_driver_init(void) 
{
	...
	/*add by rongxiao.deng.hz letting physical psensor device init first*/
/*
	 if(0==i && strcmp(alsps_init_list[i]->name,tpsensor)==0)
	 {
		 tp_flag = i;
		 continue;
	 }
...
//***************************************************
static ssize_t psensor_devinfo_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
       unsigned char *strid="TMD2772:TAOS:SUP1:";
        if(psensor_detect)
               return sprintf(buf, "%s%d\n", strid, psensor_detect);
        else
               return sprintf(buf, "NA:NA:NA:%d\n", psensor_detect);
}

static ssize_t lsensor_devinfo_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
       unsigned char *strid="TMD2772:TAOS:SUP1:";
        if(lsensor_detect)
               return sprintf(buf, "%s%d\n", strid, lsensor_detect);
        else
               return sprintf(buf, "NA:NA:NA:%d\n", lsensor_detect);
}

static DEVICE_ATTR(Psensor, 0444, psensor_devinfo_show, NULL);
static DEVICE_ATTR(Lsensor, 0444, lsensor_devinfo_show, NULL);
...
static int __init alsps_init(void) 
{
	...
	devinfo_Psensor = get_deviceinfo_dev();
	if (device_create_file(devinfo_Psensor, &dev_attr_Psensor) < 0)
	pr_err("Failed to create device file(%s)!\n", dev_attr_Psensor.attr.name);

	devinfo_Lsensor = get_deviceinfo_dev();
	if (device_create_file(devinfo_Lsensor, &dev_attr_Lsensor) < 0)
