...
static int rebootflag=0; //modify by llf for PR340854 in 20150710
int disp_bls_set_backlight(int level_1024)
{
    //modify by llf for PR340854 in 20150710 star
    if(level_1024== -1){
	rebootflag=1;
    }
    if(rebootflag==1){
	printk("[LLF_PWM] the system reboot or shutdown!-----disp_bls_set_backlight!\n");
	return 0;
    }else{ //modify by llf for PR340854 in 20150710 end
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
    ...
    //modify by llf for PR340854 in 20150710 star	
    if(rebootflag==1){
	printk("[LLF_PWM] the system reboot or shutdown!------disp_pwm_set_backlight_cmdq\n");
	return 0;
    }//modify by llf for PR340854 in 20150710 end
    if ((DISP_PWM_ALL & id) == 0) {
	...
	//add by MTK for white screen after lcm resume for defect546241
    if (old_pwm != level_1024 || (level_1024 == 0 && g_pwm_duplicate_count < 2)) {
