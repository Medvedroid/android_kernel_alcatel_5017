...
#ifdef CONFIG_COMPAT
static int bma222e_compat_ioctl(struct file *file , unsigned int cmd,unsigned long arg)
{
    long err = 0;
...
static struct file_operations bma222e_fops = {
	...
	#ifdef CONFIG_COMPAT
	.compat_ioctl = bma222e_compat_ioctl,
	#endif
...
static int bma222e_open_report_data(int open)
{
    //should queuq work to report event if  is_report_input_direct=true
    return 0;
}

static int bma222e_enable_nodata(int en)
{
    int res =0;
    int retry = 0;
    bool power=false;
    ...
    for(retry = 0; retry < 3; retry++){
        res = BMA222E_SetPowerMode(bma222e_i2c_client, power);
        if(res == 0)
        {
            GSE_LOG("BMA222E_SetPowerMode done\n");
            break;
        }
        GSE_LOG("BMA222E_SetPowerMode fail\n");
    }
    ...
    GSE_LOG("bma222e_enable_nodata OK!\n");
    return 0;
}

static int bma222e_set_delay(u64 ns)
{
    int value =0;
    value = (int)ns/1000/1000;

    GSE_LOG("bma222e_set_delay (%d), chip only use 1024HZ \n",value);
    return 0;
}

static int bma222e_get_data(int* x ,int* y,int* z, int* status)
{
    char buff[BMA222E_BUFSIZE];
    BMA222E_ReadSensorData(bma222e_i2c_client, buff, BMA222E_BUFSIZE);
...
static int bma222e_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    // driver attributes....
    ctl.open_report_data= bma222e_open_report_data;
    ctl.enable_nodata = bma222e_enable_nodata;
    ctl.set_delay  = bma222e_set_delay;
    ...
    data.get_data = bma222e_get_data;
...
#endif
static struct acc_init_info  bma222e_init_info =
    {
        .name   = BMA222E_DEV_NAME,
        .init   = bma222e_local_init,
        .uninit = bma222e_remove,
...
*/
       ...
       acc_driver_add(&bma222e_init_info);
	GSE_ERR("%s: OK\n", __func__);    
	return 0;    
}
/*----------------------------------------------------------------------------*/
