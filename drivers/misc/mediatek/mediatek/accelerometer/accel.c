...
static struct device *devinfo_gsensor;
static int gsensor_detect = 0;
...
static ssize_t gsensor_devinfo_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
       unsigned char *strid="BMA223:BOSH:SUP1:";
        if(gsensor_detect)
               return sprintf(buf, "%s%d\n", strid, gsensor_detect);
        else
               return sprintf(buf, "NA:NA:NA:%d\n", gsensor_detect);
}
static DEVICE_ATTR(gsensor, 0444, gsensor_devinfo_show, NULL);
...
static int __init acc_init(void)
{
	...
	devinfo_gsensor = get_deviceinfo_dev();
	if (device_create_file(devinfo_gsensor, &dev_attr_gsensor) < 0)
