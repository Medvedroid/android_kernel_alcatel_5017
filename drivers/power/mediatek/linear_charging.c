...
CHR_CURRENT_ENUM       g_temp_CC_value_limit = CHARGE_CURRENT_650_00_MA;
...
static BATTERY_VOLTAGE_ENUM g_cv_voltage = CV_VOLTAGE_NORMAL_TEMPERATURE;

void reduce_current_cv_on_high_temperature(void)
{
    CHR_CURRENT_ENUM max_current;

    if(BMT_status.temperature > REDUCE_CURRENT_TEMPERATURE_P3)
    {
        max_current  = MAX_CHARGE_CURRENT_OVER_TEMPERATURE_P3;
        g_cv_voltage = CV_VOLTAGE_OVER_TEMPERATURE_P3;
    }
    else if(BMT_status.temperature > REDUCE_CURRENT_TEMPERATURE_P2)
    {
        max_current  = MAX_CHARGE_CURRENT_OVER_TEMPERATURE_P2;
        g_cv_voltage = CV_VOLTAGE_OVER_TEMPERATURE_P2;
    }
    else if(BMT_status.temperature > REDUCE_CURRENT_TEMPERATURE_P1)
    {
        max_current  = MAX_CHARGE_CURRENT_OVER_TEMPERATURE_P1;
        g_cv_voltage = CV_VOLTAGE_OVER_TEMPERATURE_P1;
    }
    else
    {
        max_current  = MAX_CHARGE_CURRENT_NORMAL_TEMPERATURE;
        g_cv_voltage = CV_VOLTAGE_NORMAL_TEMPERATURE;
    }

    if(max_current < g_temp_CC_value)
    {
	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
...
#else
			{
				...
				battery_log(BAT_LOG_CRTI,
						    "[BATTERY] STANDARD_HOST CC mode charging : current=%d\r\n",
						    g_temp_CC_value);
			}
#endif
...
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 bigen*/
			battery_log(BAT_LOG_CRTI,
						    "[BATTERY] NON_STD_AC_CHARGER_CURRENT CC mode charging : current=%d\r\n",
						    g_temp_CC_value);
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 end*/							
...
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 bigen*/
			battery_log(BAT_LOG_CRTI,
						    "[BATTERY] STANDARD_CHARGER CC mode charging : current=%d\r\n",
						    g_temp_CC_value);
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 end*/							
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
...
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 bigen*/
			battery_log(BAT_LOG_CRTI,
						    "[BATTERY] CHARGING_HOST CC mode charging : current=%d\r\n",
						    g_temp_CC_value);
/*[388754]select charging current min of normal charging current and limit charging current by liming.zhu at 20150625 end*/
#endif	
        ...
        /* [834193] [MT6581][Pixi3-4NA]Support new temperature behaviour definition by [bin.song.hz] at [2014.11.7] begin */
        reduce_current_cv_on_high_temperature();
        /* [834193] [MT6581][Pixi3-4NA]Support new temperature behaviour definition by [bin.song.hz] at [2014.11.7] end */
...
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
/*[927242] [mt6735m] [pixi345] the CV value have been set in function of reduce_current_cv_on_high_temperature()  by [liming.zhu] at [201535] bigen*/
			cv_voltage = g_cv_voltage;
#else
			cv_voltage = g_cv_voltage;
#endif
/*[927242] [mt6735m] [pixi345] the CV value have been set in function of reduce_current_cv_on_high_temperature()  by [liming.zhu] at [201535] end*/
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
#endif
		}
	}
