...
kal_int32 gFG_capacity_by_bat_vol=0;
#endif
...
int battery_vender_type=0; /* 1 BYD,2 SCUD  */
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
 kal_int32 gFG_voltage_sw_ocv_flag;
 kal_int32 gFG_voltage_hw_ocv_flag;
 kal_int32 gFG_capacity_by_v_init_flag;
 void fgauge_get_battery_vender_type(void)
{
  ...
  bat_id_vol = bat_id_vol/3;
   ...
   if((bat_id_vol>=324)&&(bat_id_vol<=524))
    battery_vender_type=1;
   else if((bat_id_vol>=1243)&&(bat_id_vol<=1443))
   	{
   		battery_vender_type=2;
   	}
   else
   	{
   		printk("[fgauge_get_battery_vender_type] get battery vender type error\n");
   	}
   printk("[fgauge_get_battery_vender_type] battery_vender_type=%d\n",battery_vender_type );
...
if(battery_vender_type=1) //byd battery 
{
	if (temperature <= TEMPERATURE_T1) {
		low_temperature = (-10);
		low_Q_max = Q_MAX_NEG_BYD_10;
		high_temperature = TEMPERATURE_T1;
		high_Q_max = Q_MAX_POS_BYD_0;
		...
		low_temperature = TEMPERATURE_T1;
		low_Q_max = Q_MAX_POS_BYD_0;
		high_temperature = TEMPERATURE_T2;
		high_Q_max = Q_MAX_POS_BYD_25;
		...
		low_temperature = TEMPERATURE_T2;
		low_Q_max = Q_MAX_POS_BYD_25;
		high_temperature = TEMPERATURE_T3;
		high_Q_max = Q_MAX_POS_BYD_50;
...
else
{
	if (temperature <= TEMPERATURE_T1) {
		low_temperature = (-10);
		low_Q_max = Q_MAX_NEG_SCUD_10;
		high_temperature = TEMPERATURE_T1;
		high_Q_max = Q_MAX_POS_SCUD_0;
		...
		low_temperature = TEMPERATURE_T1;
		low_Q_max = Q_MAX_POS_SCUD_0;
		high_temperature = TEMPERATURE_T2;
		high_Q_max = Q_MAX_POS_SCUD_25;
		...
		low_temperature = TEMPERATURE_T2;
		low_Q_max = Q_MAX_POS_SCUD_25;
		high_temperature = TEMPERATURE_T3;
		high_Q_max = Q_MAX_POS_SCUD_50;
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
   ...
   if(battery_vender_type=1) //byd battery
	...
	if (temperature <= TEMPERATURE_T1) {
		low_temperature = (-10);
		low_Q_max = Q_MAX_NEG_10_H_BYD_CURRENT;
		high_temperature = TEMPERATURE_T1;
		high_Q_max = Q_MAX_POS_0_H_BYD_CURRENT;
		...
		low_temperature = TEMPERATURE_T1;
		low_Q_max = Q_MAX_POS_0_H_BYD_CURRENT;
		high_temperature = TEMPERATURE_T2;
		high_Q_max = Q_MAX_POS_25_H_BYD_CURRENT;
		...
		low_temperature = TEMPERATURE_T2;
		low_Q_max = Q_MAX_POS_25_H_BYD_CURRENT;
		high_temperature = TEMPERATURE_T3;
		high_Q_max = Q_MAX_POS_50_H_BYD_CURRENT;
...
else
{
	if (temperature <= TEMPERATURE_T1) {
		low_temperature = (-10);
		low_Q_max = Q_MAX_NEG_10_H_SCUD_CURRENT;
		high_temperature = TEMPERATURE_T1;
		high_Q_max = Q_MAX_POS_0_H_SCUD_CURRENT;
		...
		low_temperature = TEMPERATURE_T1;
		low_Q_max = Q_MAX_POS_0_H_SCUD_CURRENT;
		high_temperature = TEMPERATURE_T2;
		high_Q_max = Q_MAX_POS_25_H_SCUD_CURRENT;
		...
		low_temperature = TEMPERATURE_T2;
		low_Q_max = Q_MAX_POS_25_H_SCUD_CURRENT;
		high_temperature = TEMPERATURE_T3;
		high_Q_max = Q_MAX_POS_50_H_SCUD_CURRENT;
...
int fgauge_get_saddles(void)
{
	...
	return sizeof(battery_profile_byd_t2) / sizeof(BATTERY_PROFILE_STRUC);
     else
	 return sizeof(battery_profile_scud_t2) / sizeof(BATTERY_PROFILE_STRUC);
}

int fgauge_get_saddles_r_table(void)
{
     if(battery_vender_type=1) //byd battery 
	return sizeof(r_profile_byd_t2) / sizeof(R_PROFILE_STRUC);
     else
	return sizeof(r_profile_scud_t2) / sizeof(R_PROFILE_STRUC);
}
   ...
   if(battery_vender_type=1) //byd battery 
   	{
   	  switch (temperature) {
	case TEMPERATURE_T0:
		return &battery_profile_byd_t0[0];
		break;
	case TEMPERATURE_T1:
		return &battery_profile_byd_t1[0];
		break;
	case TEMPERATURE_T2:
		return &battery_profile_byd_t2[0];
		break;
	case TEMPERATURE_T3:
		return &battery_profile_byd_t3[0];
		break;
	case TEMPERATURE_T:
		return &battery_profile_byd_temperature[0];
		break;
	default:
		return NULL;
		break;	
	...
	switch (temperature) {
	case TEMPERATURE_T0:
		return &battery_profile_scud_t0[0];
		break;
	case TEMPERATURE_T1:
		return &battery_profile_scud_t1[0];
		break;
	case TEMPERATURE_T2:
		return &battery_profile_scud_t2[0];
		break;
	case TEMPERATURE_T3:
		return &battery_profile_scud_t3[0];
		break;
	case TEMPERATURE_T:
		return &battery_profile_scud_temperature[0];
		break;
	default:
		return NULL;
		break;
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_uint32 temperature)
{
  if(battery_vender_type=1)
  	{
  	   	switch (temperature) {
	case TEMPERATURE_T0:
		return &r_profile_byd_t0[0];
		break;
	case TEMPERATURE_T1:
		return &r_profile_byd_t1[0];
		break;
	case TEMPERATURE_T2:
		return &r_profile_byd_t2[0];
		break;
	case TEMPERATURE_T3:
		return &r_profile_byd_t3[0];
		break;
	case TEMPERATURE_T:
		return &r_profile_byd_temperature[0];
		break;
	default:
		return NULL;
		break;
...
else
{
	switch (temperature) {
	case TEMPERATURE_T0:
		return &r_profile_scud_t0[0];
		break;
	case TEMPERATURE_T1:
		return &r_profile_scud_t1[0];
		break;
	case TEMPERATURE_T2:
		return &r_profile_scud_t2[0];
		break;
	case TEMPERATURE_T3:
		return &r_profile_scud_t3[0];
		break;
	case TEMPERATURE_T:
		return &r_profile_scud_temperature[0];
		break;
	default:
		return NULL;
		break;
...
/*[212006] The capacity diaplay abnormally by liming.zhu at 20150506 bigen*/
		for(j=51;j< saddles-1;j++)
			{
			  (profile_p[i]+j) -> voltage = (temp_profile_p + 50) -> voltage;
			(profile_p[i]+j) -> percentage = (temp_profile_p + 50) -> percentage;
			}
/*[212006] The capacity diaplay abnormally by liming.zhu at 20150506 end*/	
	}
...
void dod_init(void)
{
    ...
    /* [101253] fix "gFG_voltage" value too large campare with "gFG_capacity_by_v_init" by liming.zhu.hz at [201501114] begin*/
    if (abs(gFG_capacity_by_v_init - gFG_capacity_by_v) > 5)
       {
           gFG_capacity_by_v = gFG_capacity_by_v_init;
    }
    /* [101253] fix "gFG_voltage" value too large campare with "gFG_capacity_by_v_init" by liming.zhu.hz at [201501114] end*/
	
	bm_print(BM_LOG_CRTI, "[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
		 gFG_voltage, gFG_capacity_by_v, gFG_capacity_by_v_init);
	gFG_capacity_by_v_init_flag=gFG_capacity_by_v;
	gFG_voltage_hw_ocv_flag=gFG_voltage;
...
/*[326405] battery dicharged too fast by liming.zhu.hz at 20150614 bigen*/
#ifdef CONFIG_CAR_TUNE_VALUE_Check
	gFG_capacity_by_bat_vol = fgauge_read_capacity_by_v(BMT_status.bat_vol);
#endif
/*[326405] battery dicharged too fast by liming.zhu.hz at 20150614 end*/
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
...
/* 2. Calculate battery capacity by VBAT */
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	gFG_voltage_sw_ocv_flag=gFG_voltage;
	gFG_capacity_by_v_init = gFG_capacity_by_v;
...
#endif
           fgauge_get_battery_vender_type();
#if defined(SOC_BY_AUXADC)
		g_auxadc_solution = 1;
		table_init();
...
/* add by bin.song.hz begin*/
int battery_is_file_ready(const char* path){
    mm_segment_t oldfs;
    int ret = 0;
    struct file* filp;
    ...
    filp = filp_open(path, O_RDONLY|O_CREAT, 0);
    if(IS_ERR(filp))
    {
        ret = -1;
        printk("%s %i file is not ready %s\n", __func__, __LINE__, path);
    }
    else
    {
...
int battery_write_file(const char* path, const void* buf, size_t size){
    ssize_t ret = 0;
    mm_segment_t oldfs;
    loff_t fpos = 0;
    struct file* filp = NULL;
    const char* wbuf = (const char *)buf;
    ...
    {
        ret = -1;
        printk("%s %i fail to open file %s\n", __func__, __LINE__, path);
    }else
    {
        if((ret = vfs_write(filp, wbuf, size, &fpos)) < 0)
        {
            printk("%s %i fail to write file %s\n", __func__, __LINE__, path);
        }
        filp_close(filp, NULL);
    }
...
int battery_read_file(const char* path, void* buf, size_t size){
    ssize_t ret = 0;
    mm_segment_t oldfs;
    loff_t fpos = 0;
    struct file* filp = NULL;
    ...
    {
        if((ret = vfs_read(filp, wbuf, size, &fpos)) < 0)
        {
            printk("%s %i fail to read file %s\n", __func__, __LINE__, path);
        }
        filp_close(filp, NULL);
    }
 ...
 kal_bool battery_read_tune_cfg(int * val)
{
    if(battery_is_file_ready(BAT_CFG_PATH) < 0)
    {
        printk("%s %i battery cfg file is not ready\n", __func__, __LINE__);
        return KAL_FALSE;
    }

    if(battery_read_file(BAT_CFG_PATH, (void *)val, sizeof(int)) < 0){
        printk("%s %i failed to read cfg file\n", __func__, __LINE__);
        return KAL_FALSE;
    }else{
        printk("%s %i read capacity tune value = %d\n", __func__, __LINE__, *val);
        return KAL_TRUE;
    }
}

kal_bool battery_write_tune_cfg(int val)
{
    if(battery_is_file_ready(BAT_CFG_PATH) < 0)
    {
        printk("%s %i battery cfg file is not ready\n", __func__, __LINE__);
    ...
    if(battery_write_file(BAT_CFG_PATH, (void *)&val, sizeof(int)) < 0){
        printk("%s %i failed to write cfg file\n", __func__, __LINE__);
        return KAL_FALSE;
    }else{
        printk("%s %i write capacity tune value = %d\n", __func__, __LINE__, val);
        return KAL_TRUE;
    }
}
/* add by bin.song.hz end*/

//for  CAR_TUNE_VALUE  too high make it low ,check it  in charge full point  soc>=99
kal_bool battery_car_tune_low_check_in_chargefull(void) 
{
	kal_bool car_tune_value_change=KAL_FALSE;
	kal_int32 Init_SOC= 100-gFG_DOD0;
	if(Init_SOC<15)
	{
			if(BMT_status.ICharging>=250)
			{
			 	static int counter_ICharge=0;
				if (counter_ICharge >= 3)
				{
					CAR_TUNE_VALUE--;
					#ifndef CAR_TUNE_VALUE_Debug
					if(CAR_TUNE_VALUE > MAX_CAR_TUNE_VALUE)
				            CAR_TUNE_VALUE=MAX_CAR_TUNE_VALUE;
				          else if (CAR_TUNE_VALUE < MIN_CAR_TUNE_VALUE)
				               CAR_TUNE_VALUE=MIN_CAR_TUNE_VALUE;
					 #endif
					  battery_write_tune_cfg(CAR_TUNE_VALUE);
					     counter_ICharge=0;	
					//resetBatteryMeter=KAL_TRUE;
					printk("Init_SOC=%d CAR_TUNE_VALUE is high  counter_ICharge=%d,CAR_TUNE_VALUE=%d\n",Init_SOC,counter_ICharge,CAR_TUNE_VALUE);
					car_tune_value_change=KAL_TRUE; 
                 			 }else
					{
						counter_ICharge++;
					}	
	...
	return car_tune_value_change ;
}

//for  CAR_TUNE_VALUE  too low make it high,check it  in charge full point 
kal_bool battery_car_tune_high_check_in_chargefull(void) 
{
kal_bool car_tune_value_change=KAL_FALSE ;
kal_int32 Init_SOC= 100-gFG_DOD0;
if(Init_SOC<15)
	{
	if(BMT_status.bat_full==KAL_TRUE) //soc <99
		{
			int NEW_CAR_TUNE_VALUE=80;
			int delta_capacity_temp=0;	
			delta_capacity_temp=BMT_status.SOC-Init_SOC;
			NEW_CAR_TUNE_VALUE=(100-Init_SOC)*CAR_TUNE_VALUE/delta_capacity_temp;
			printk("Init_SOC=%d CAR_TUNE_VALUE is low, NEW_CAR_TUNE_VALUE=%d,CAR_TUNE_VALUE=%d\n",Init_SOC,NEW_CAR_TUNE_VALUE,CAR_TUNE_VALUE);
			CAR_TUNE_VALUE=NEW_CAR_TUNE_VALUE;
			#ifndef CAR_TUNE_VALUE_Debug
				if(CAR_TUNE_VALUE > MAX_CAR_TUNE_VALUE)
					CAR_TUNE_VALUE=MAX_CAR_TUNE_VALUE;
				else if (CAR_TUNE_VALUE < MIN_CAR_TUNE_VALUE)
...
//for  CAR_TUNE_VALUE  too low make it high,check it  in low battery percentage 
kal_bool battery_car_tune_high_check_in_low_percentage(void) //soc >1
{
	kal_bool car_tune_value_change=KAL_FALSE ;
	kal_int32 Init_SOC= 100-gFG_DOD0;
	if(Init_SOC>85)	
            {	
 		if((BMT_status.bat_vol <= V_0PERCENT_TRACKING)&&(BMT_status.SOC>=3)&&(BMT_status.SOC<Init_SOC))
		{
			int NEW_CAR_TUNE_VALUE=80;
			int delta_capacity_temp=0;
			delta_capacity_temp=Init_SOC-BMT_status.SOC;
			NEW_CAR_TUNE_VALUE=(Init_SOC-1)*CAR_TUNE_VALUE/delta_capacity_temp;
			printk("Init_SOC=%d CAR_TUNE_VALUE is low, NEW_CAR_TUNE_VALUE=%d,CAR_TUNE_VALUE=%d\n",Init_SOC,NEW_CAR_TUNE_VALUE,CAR_TUNE_VALUE);
			CAR_TUNE_VALUE=NEW_CAR_TUNE_VALUE;
			#ifndef CAR_TUNE_VALUE_Debug
				if(CAR_TUNE_VALUE > MAX_CAR_TUNE_VALUE)
...
//for  CAR_TUNE_VALUE  too high make it low,check it  in 1percentage
kal_bool battery_car_tune_low_check_in_1percentage(void)  // soc=1
{
kal_bool car_tune_value_change=KAL_FALSE ;
kal_int32 Init_SOC= 100-gFG_DOD0;
if(Init_SOC>85)
{
		printk(" gFG_capacity_by_bat_vol=%d,BMT_status.bat_vol=%d\n",gFG_capacity_by_bat_vol,BMT_status.bat_vol);
		if ((gFG_capacity_by_bat_vol >= 3)&&(BMT_status.bat_vol >V_0PERCENT_TRACKING))
		{					
			CAR_TUNE_VALUE --;
			#ifndef CAR_TUNE_VALUE_Debug
				if(CAR_TUNE_VALUE > MAX_CAR_TUNE_VALUE)
			...
			#endif
			battery_write_tune_cfg(CAR_TUNE_VALUE);
			printk("Init_SOC=%d, CAR_TUNE_VALUE is high, CAR_TUNE_VALUE=%d\n",Init_SOC,CAR_TUNE_VALUE);
			car_tune_value_change=KAL_TRUE;
				//resetBatteryMeter=KAL_TRUE;
		}
}
